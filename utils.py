...
def visualize_population(population, criteria):
    for pop in population:
        dict_population = {k: v for v, k in enumerate(pop)}
        dict_population = dict(sorted(dict_population.items()))
        for idx, order in enumerate(dict_population.values()):
            print(order, end=' ')
            if idx == criteria:
                print('\n')
                break